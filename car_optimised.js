class Car {
  ...
  drive(see, view, time, best) {
    var wll = this.sense(); //detects dist
    this.progress(wll, time, best); //detects if crashed and checkpoints crossed
    if(see)
      this.show(view);
  ...
  progress(wll, time, best) {
    ...
    */
    ...
    //CHECK IF CRASHED
    let m = max(wls1.length, wls2.length);
    this.crashed = false;
    this.color = color(255, 204, 0);
    for (var i = 0; i < m && !this.crashed; i++) {
      if (i < wls1.length) {
        for (var j = 0; j < 4; j++) {
          let intr = this.line_intersect(cx[j], cy[j], cx[(j + 1) % 4], cy[(j + 1) % 4], walls1[wls1[i][0]][0], walls1[wls1[i][0]][1], walls1[wls1[i][1]][0], walls1[wls1[i][1]][1]);
          if (intr != null) {
            this.crashed = true;
      ...
      if (!this.crashed && i < wls2.length) {
        for (var j = 0; j < 4; j++) {
          let intr = this.line_intersect(cx[j], cy[j], cx[(j + 1) % 4], cy[(j + 1) % 4], walls2[wls2[i][0]][0], walls2[wls2[i][0]][1], walls2[wls2[i][1]][0], walls2[wls2[i][1]][1]);
          if (intr != null) {
            this.crashed = true;
            this.color = color(255, 0, 0);
            break;
    ...
    */
    ...
    if(best) {
      if(this.chkpt_crossed >= floor(best.chkpt_crossed / 2)) {
        ...
        if(!this.crashed)
          this.score += 1;
        var laps_done = floor((this.chkpt_crossed-1) / floor((this.chkpts_rem.length + this.chkpts_com.length)/2));
        if (laps_done > 0)
          this.score *= pow(5,Math.log10(10*laps_done));
      }
    } else {
      var i = this.chkpt_crossed;
      while(i > 1) {
